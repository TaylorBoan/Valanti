...
function App() {
        ...
        if (result.length) {
          setSelectedModel((current) => {
            ...
            const fallback = result.find(
              (model) => model.hasPriceData !== false
            );
            return fallback?.key ?? "";
          });
        } else {
  ...
  useEffect(() => {
    ...
    const loadMetrics = async () => {
      ...
      setError(null);
      try {
        const summary = await api.getSummaryMetrics(selectedModel);
        if (isActive) {
          setMetrics(summary);
        }
      } catch (err) {
  ...
  const activeModelLabel = activeModel
    ? `${activeModel.make} ${activeModel.label}`
    : null;
  ...
  return (
    <div className="dashboard">
      <header className="hero">
        ...
        <h1>Valanti</h1>
        <p>
          250k+ scraped listings from AutoTempest unified in a single source of
          truth. Track live asking prices across the rarest models and spot
          directionally where the market is headed.
        </p>
        {metrics && (
          <p className="hero__meta">
            Data refreshed {formatDate(metrics.updatedAt)}
          </p>
        )}
      </header>
      ...
      <section className="kpi-grid">
        ...
        <KpiCard
          title="Unique VINs"
          value={formatNumber(metrics?.uniqueVins)}
        />
        <KpiCard
        ...
        <KpiCard
          ...
          subtitle={
            activeModelLabel
              ? `${activeModelLabel} · prior 30 days`
              : "Prior 30-day window"
          }
      ...
      <section className="panel">
        <div className="panel__header">
          <div>
            ...
            <h2>{activeModelLabel ?? "Select a model"}</h2>
            <p className="panel__subtitle">
              {priceHistory?.stats.totalPoints ? (
                <>
                  {priceHistory.stats.totalPoints} observations collected · Last
                  updated {formatDate(priceHistory.stats.lastUpdated)}
                </>
              ) : (
